    ...
    <template v-else>
      ...
      <el-card shadow="never">
        ...
        <el-table :data="filteredRules" size="small"
          ...
          <el-table-column type="index" :label="t('gas.rules.colIndex')" width="60" align="center" />
          <el-table-column :label="t('gas.rules.colLevel')" width="90" align="center">
          ...
          <el-table-column :label="t('gas.rules.colChannels')" width="200">
            <template #default="{ row }">
          ...
          <el-table-column :label="t('gas.rules.colTriggerCond')" min-width="200">
            <template #default="{ row }">
              ...
              <div class="condition-tags">
                ...
                <span v-if="ruleTriggerTags(row).length === 0" class="text-secondary">{{ t('gas.rules.noCondition') }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="t('gas.rules.colAction')" width="150" align="center">
            <template #default="{ row }">
              <!-- [FEAT 2026-09-02] 单条就地编辑: 点哪条只编辑哪条 (动作编排仍在平台页) -->
              ...
              <el-button size="small" link type="primary" @click="openTriggerDetail(row)">{{ t('gas.rules.triggerDetail') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
 ...
 * 本页为规则实例聚合视图, 展示后端已配置模板, 不新增/修改动作
 * (启停开关只改 enabled 布防状态, 不触碰动作编排, 红线不受影响)。
 */
...
async function toggleRule(rule: LinkageRule) {
  ...
  try {
    ...
    rule.enabled = next
    const stateTxt = next ? t('gas.rules.stateOn', '启用') : t('gas.rules.stateOff', '停用')
    ElMessage.success(t('gas.rules.toggleOk', `规则「${rule.name || rule.id}」已${stateTxt}`))
  } catch (e: unknown) {
    const msg = (e as Error)?.message ?? String(e)
    ElMessage.error(t('gas.rules.toggleFail', `规则「${rule.name || rule.id}」启停失败: ${msg}`))
  } finally {
    togglingId.value = ''
  }
}
...
onMounted(() => {
  ...
  ensureEventTypesZh() // [UX-ZH] 事件类型中文名预热 (canonical SSOT)
})
</script>
