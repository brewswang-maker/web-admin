<template>
  <div class="scr-events-page">
   <!-- [UI-4b 2026-09-10] 左侧设备树筛选面板 (统一规则: 设备列表居左/主内容居右) -->
   <AlarmDeviceTreePanel @selection-change="onTreeSelection" />

   <div class="scr-events-main">
    <!-- ===== 场景事件类型筛选 (SSOT metadata?scene=security_screening 动态拉取) ===== -->
    <el-card shadow="never" class="filter-card">
      <div class="scene-bar">
        <span class="bar-label">安检事件</span>
    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <div class="header-right">
            ...
            <AlarmViewToggle v-model="viewMode" page-key="screening" />
      ...
      <!-- [P2] 卡片视图 (AlarmCard 栅格; 筛选/分页逻辑零改动) -->
      ...
      <el-table v-else :data="pagedFinal" v-loading="loading" size="small"
                :empty-text="selectedType ? '该类型暂无事件' : '暂无安检相关事件'">
...
/**
 * 安检事件列表 — [UI-4b 2026-09-10]
 *
 * 事件类型筛选 chips 从 /event-types/metadata?scene=security_screening 动态拉取 (SSOT),
...
const SCENE_TAG = 'security_screening'
...
// 卡片/列表视图 (持久化 key alarm_view_mode_screening, 与 AlarmViewToggle 同规范)
const viewMode = ref<'card' | 'table'>(
  localStorage.getItem('alarm_view_mode_screening') === 'card' ? 'card' : 'table'
)
...
function chipClass(alarmType: string) {
  if (alarmType.includes('knife') || alarmType.includes('gun')) return 'chip-red'
  return ''
}
...
async function fetchEvents() {
  loading.value = true
  try {
    // 场景过滤下沉服务端 (scene=security_screening, 与 scene_tags 登记自动同步)
    const res = await alarmApi.getList({ page: 1, pageSize: 500, scene: SCENE_TAG })
    // 统一走 normalizeAlarmCore (SSOT, 与周界/大型活动同范式)
    events.value =
...
<style scoped>
.scr-events-page { padding: 4px 0; display: flex; gap: 12px; align-items: flex-start; }
.scr-events-main { flex: 1; min-width: 0; }
.events-card-grid {
  display: grid;
