    ...
    <template v-else>
      ...
      <el-card shadow="never">
        ...
        <el-table :data="filteredRules" size="small"
          ...
          <el-table-column type="index" :label="t('hotel.rules.colIndex')" width="60" align="center" />
          <el-table-column :label="t('hotel.rules.colLevel')" width="90" align="center">
          ...
          <el-table-column :label="t('hotel.rules.colChannels')" width="200">
            <template #default="{ row }">
          ...
          <el-table-column :label="t('hotel.rules.colTriggerCond')" min-width="200">
            <template #default="{ row }">
              ...
              <div class="condition-tags">
                ...
                <span v-if="ruleTriggerTags(row).length === 0" class="text-secondary">{{ t('hotel.rules.noCondition') }}</span>
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="t('hotel.rules.colAction')" width="150" align="center">
            <template #default="{ row }">
              ...
              <el-button size="small" link type="primary" @click="openTriggerDetail(row)">{{ t('hotel.rules.triggerDetail') }}</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
...
async function toggleRule(rule: LinkageRule) {
  ...
  try {
    ...
    rule.enabled = next
    const stateTxt = next ? t('hotel.rules.stateOn', '启用') : t('hotel.rules.stateOff', '停用')
    ElMessage.success(t('hotel.rules.toggleOk', `规则「${rule.name || rule.id}」已${stateTxt}`))
  } catch (e: unknown) {
    const msg = (e as Error)?.message ?? String(e)
    ElMessage.error(t('hotel.rules.toggleFail', `规则「${rule.name || rule.id}」启停失败: ${msg}`))
  } finally {
    togglingId.value = ''
  }
}
